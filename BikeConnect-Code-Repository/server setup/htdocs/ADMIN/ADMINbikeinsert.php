...
try {
    ...
    if (isset($_POST['EID'])&&isset($_POST['TOKEN'])) {
        if(isset($_POST['BTYPE'])&&isset($_POST['STATUS'])&&isset($_POST['VID'])&&isset($_POST['HID'])&&isset($_POST['ADDRESS'])){
        $EID =
        ...
        $STATUS =
         filter_var( $_POST['STATUS'],FILTER_SANITIZE_SPECIAL_CHARS);
                 $VID =
         filter_var($_POST['VID'],FILTER_SANITIZE_SPECIAL_CHARS);
		          $HID =
         filter_var($_POST['HID'],FILTER_SANITIZE_SPECIAL_CHARS);
        $ADDRESS =
         filter_var( $_POST['ADDRESS'],FILTER_SANITIZE_SPECIAL_CHARS);
                $sql = 
                //dont use ` for sending this...
                'INSERT INTO public."BIKES"("BTYPE", "STATUS", "VID", "HID", "ADDRESS")VALUES('."'".$BTYPE."','".$STATUS."','".$VID."','".$HID."','".$ADDRESS."');";
				$dbquery=$sql;                
				$ret = pg_query($db, $sql);
                 if (!$ret) {
