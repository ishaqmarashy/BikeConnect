    ...
    try{
            if($conflicts==false){
                       ...
                       foreach($conflicts as $key=>$value) {
                        ...
                        $sql = 
                        'SELECT "BID" FROM public."BIKEHUBVIEW" WHERE "HID"='."'".$HID."'"
                        .' AND "BTYPE"='."'".$BTYPE."' " .' AND "STATUS"='."'FUNCTIONING'" 
                        .$i ;
                         $ret = pg_query($db, $sql);
                        if (!$ret) {
                           return false;
...
function MakeBooking($USERNAME,$DATEFROM,$DATETO,$BOOKINGTYPE, $GEAR,$db){
    try{
             $sql = 
                                            'INSERT INTO public."BOOKING" ("USERNAME", "DATEFROM", "DATETO", "BOOKINGTYPE","GEAR")VALUES ('
                                             ."'".$USERNAME."','".$DATEFROM."','".$DATETO."','".$BOOKINGTYPE."','".$GEAR."');";
                                             $ret = pg_query($db, $sql);
                                             if (!$ret) {
                                                return false;
                                             } else {
...
function getBOID($USERNAME,$DATEFROM,$DATETO,$BOOKINGTYPE,$HID,$db){
    try{
             $sql = 
                                            'SELECT "BOID" FROM public."BOOKING" WHERE "USERNAME"='."'".$USERNAME."'".' AND "DATEFROM"= '."'".$DATEFROM."'".' AND   "DATETO"='."'".$DATETO."'".' AND "BOOKINGTYPE"='."'".$BOOKINGTYPE."'";
                                             $ret = pg_query($db, $sql);
                                             if (!$ret) {
                                                return false;
                                             } else {
...
function assignBikeToBooking($BOID,$availablebikes,$QUANTITY,$db){
    try{
        $sql =' ';
        $i=0;
        while ($i <$QUANTITY){
         $sql =$sql .'INSERT INTO public."BB" ( "BOID", "BID" ) VALUES ('."'".$BOID."','".$availablebikes[$i]['BID']."');";
         $i++;
        }
